...
# 코드는 01_code/ 아래(CODE_DIR), 데이터·모델·static 은 프로젝트 루트(BASE_DIR)에 위치한다.
CODE_DIR = Path(__file__).resolve().parent
BASE_DIR = CODE_DIR.parent
DATASETS_DIR = BASE_DIR / "07_app" / "static" / "datasets"
PIPELINE = CODE_DIR / "pipeline.py"
...
app = Flask(__name__, static_folder=str(BASE_DIR / "07_app" / "static"), static_url_path="/static")
...
def api_ingest():
    ...
    if body.get("auto_label"):
        model = BASE_DIR / "07_app" / "models" / "autolabel_model.pkl"
        if not model.exists():
